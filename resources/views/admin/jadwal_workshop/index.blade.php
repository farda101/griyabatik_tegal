...
@section('title', 'Manajemen Jadwal Workshop')
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                    <div class="flex items-center">
                        {{-- Ikon Jadwal Workshop --}}
                        <div class="bg-gradient-to-r from-teal-500 to-cyan-500 p-2 rounded-lg mr-3">
                            <svg class="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                            </svg>
                        </div>
                        <h1 class="text-3xl font-bold text-gray-900">Manajemen Jadwal Workshop</h1>
                    </div>
                    <div class="mt-4 sm:mt-0">
                        <a href="{{ route('admin.jadwal_workshop.create') }}"
                        ...
                        @forelse ($jadwalWorkshops as $jadwal)
                            <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    {{ $loop->iteration + ($jadwalWorkshops->currentPage() - 1) * $jadwalWorkshops->perPage() }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $jadwal->paketWorkshop->nama_paket ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    {{ $jadwal->max_peserta }} <span class="text-gray-500">orang</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    {{ $jadwal->peserta_terdaftar }} <span class="text-gray-500">orang</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    ...
                                    @php
                                        ...
                                        if ($jadwal->status === 'available') {
                                            ...
                                            $statusText = 'Tersedia';
                                        } elseif ($jadwal->status === 'unavailable') {
                                    ...
                                    @endphp
                                    <form action="{{ route('admin.jadwal_workshop.toggleStatus', $jadwal->id) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusClass }} transition duration-150 ease-in-out">
                                            {{ $statusText }}
                                        </button>
                                    </form>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('admin.jadwal_workshop.edit', $jadwal->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-4 transition duration-150 ease-in-out">
                                    ...
                                    <form action="{{ route('admin.jadwal_workshop.destroy', $jadwal->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus jadwal workshop ini? Ini mungkin terikat dengan reservasi yang sudah ada dan aksi ini tidak dapat dibatalkan!');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                    Belum ada data jadwal workshop.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
