            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                    <div class="flex items-center">
                        ...
                        <h1 class="text-3xl font-bold text-gray-900">Manajemen Video Edukasi</h1>
                    </div>
                        ...
                        @forelse ($videos as $video)
                            <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    @if($video->thumbnail)
                                        <img src="{{ asset('storage/' . $video->thumbnail) }}" alt="{{ $video->judul }}" class="h-16 w-24 object-cover rounded-md shadow-sm">
                                    @else
                                        <div class="h-16 w-24 bg-gray-100 flex items-center justify-center rounded-md text-gray-400 text-xs">No Thumb</div>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <form action="{{ route('admin.video.toggleStatus', $video->id) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('PATCH')
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('admin.video.show', $video->id) }}" class="text-blue-600 hover:text-blue-900 mr-4 transition duration-150 ease-in-out">
                                    ...
                                    <a href="{{ route('admin.video.edit', $video->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-4 transition duration-150 ease-in-out">
                                        <svg class="h-5 w-5 inline-block -mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                        Edit
                                    </a>
                                    <form action="{{ route('admin.video.destroy', $video->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus video ini? File video dan thumbnail juga akan dihapus dan aksi ini tidak dapat dibatalkan!');">
                                        @csrf
                                        @method('DELETE')
