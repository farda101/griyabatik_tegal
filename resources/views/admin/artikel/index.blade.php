                            ...
                            <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    @if($artikel->featured_image)
                                        <img src="{{ asset('storage/' . $artikel->featured_image) }}" alt="{{ $artikel->judul }}" class="h-16 w-24 object-cover rounded-md shadow-sm">
                                    @else
                                        <div class="h-16 w-24 bg-gray-100 flex items-center justify-center rounded-md text-gray-400 text-xs">No Image</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm font-medium text-gray-900 max-w-xs overflow-hidden text-ellipsis">
                                    <p class="font-semibold">{{ $artikel->judul }}</p>
                                    <p class="text-xs text-gray-500 italic mt-1">{{ Str::limit($artikel->excerpt, 50) }}</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    {{ $artikel->author->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <form action="{{ route('admin.artikel.toggleStatus', $artikel->id) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full
                                            {{ $artikel->status === 'published' ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' }}
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    {{ $artikel->published_at ? \Carbon\Carbon::parse($artikel->published_at)->format('d M Y') : '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('admin.artikel.show', $artikel->id) }}" class="text-blue-600 hover:text-blue-900 mr-4 transition duration-150 ease-in-out">
                                    ...
                                    <a href="{{ route('admin.artikel.edit', $artikel->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-4 transition duration-150 ease-in-out">
                                        <svg class="h-5 w-5 inline-block -mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                        Edit
                                    </a>
                                    <form action="{{ route('admin.artikel.destroy', $artikel->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus artikel ini? Gambar unggulan juga akan dihapus dan aksi ini tidak dapat dibatalkan!');">
                                        @csrf
                                        @method('DELETE')
