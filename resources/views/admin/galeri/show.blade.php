        ...
        <div class="mb-8">
            <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                <div class="flex items-center">
                    ...
                    <h1 class="text-3xl font-bold text-gray-900">Detail Foto Galeri</h1>
                </div>
                <p class="text-gray-600 mt-2 ml-12">Untuk foto: <span class="font-semibold text-teal-700">{{ $galeri->judul }}</span></p>
            </div>
        </div>

        {{-- Main Content Card with Details --}}
        <div class="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
            {{-- Bagian Pratinjau Foto --}}
            <div class="mb-8 p-4 bg-gray-50 rounded-lg border border-gray-100 shadow-sm text-center">
                <h3 class="font-bold text-xl text-gray-900 mb-4 pb-2 border-b-2 border-teal-200 inline-flex items-center">
                ...
                <div class="mt-4">
                    @if($galeri->foto)
                        <img src="{{ asset('storage/' . $galeri->foto) }}" alt="{{ $galeri->judul }}" class="mx-auto max-w-full h-auto max-h-96 object-contain rounded-lg shadow-md border border-gray-300">
                    @else
                        <div class="bg-gray-100 flex items-center justify-center h-48 rounded-lg border border-gray-300 text-gray-500 text-lg font-medium">
                            Tidak ada foto
                        </div>
                    @endif
                </div>
            ...
            <div class="p-4 bg-gray-50 rounded-lg border border-gray-100 shadow-sm">
                ...
                <dl class="divide-y divide-gray-200 text-gray-700">
                    <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        <dt class="text-sm font-medium">ID Foto</dt>
                        <dd class="mt-1 text-sm sm:col-span-2 sm:mt-0 font-semibold text-gray-800">{{ $galeri->id }}</dd>
                    </div>
                    <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        <dt class="text-sm font-medium">Judul</dt>
                        <dd class="mt-1 text-sm sm:col-span-2 sm:mt-0">{{ $galeri->judul }}</dd>
                    </div>
                    <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        <dt class="text-sm font-medium">Deskripsi</dt>
                        <dd class="mt-1 text-sm sm:col-span-2 sm:mt-0">{{ $galeri->deskripsi ?? '-' }}</dd>
                    </div>
                    <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        <dt class="text-sm font-medium">Urutan Tampilan</dt>
                        <dd class="mt-1 text-sm sm:col-span-2 sm:mt-0">{{ $galeri->urutan }}</dd>
                    </div>
                    <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        <dt class="text-sm font-medium">Status</dt>
                        <dd class="mt-1 text-sm sm:col-span-2 sm:mt-0">
                            @php
                                $statusClass = $galeri->is_active ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-red-100 text-red-800 border border-red-200';
                            @endphp
                    ...
                    <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        ...
                        <dd class="mt-1 text-sm sm:col-span-2 sm:mt-0">{{ \Carbon\Carbon::parse($galeri->created_at)->format('d M Y H:i') }}</dd>
                    </div>
                    <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                        <dt class="text-sm font-medium">Terakhir Diperbarui</dt>
                        <dd class="mt-1 text-sm sm:col-span-2 sm:mt-0">{{ \Carbon\Carbon::parse($galeri->updated_at)->format('d M Y H:i') }}</dd>
                    </div>
                </dl>
            </div>
