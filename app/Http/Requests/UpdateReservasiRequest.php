...
class UpdateReservasiRequest extends FormRequest
{
    ...
    {
        // Hanya superadmin yang diizinkan untuk memperbarui reservasi
    ...
    {
        // Parameter reservasi dari route model binding
        $reservasi = $this->route('reservasi');

        return [
            // Jadwal workshop ID dan jenis peserta mungkin tidak bisa diubah setelah reservasi dibuat,
            // atau bisa diubah tapi dengan validasi kompleks. Untuk sederhana, kita buat required.
            'jadwal_workshop_id' => 'required|exists:jadwal_workshops,id',
            ...
            'file_permohonan' => 'nullable|file|mimes:pdf,doc,docx|max:2048', // Bisa diubah/di-upload ulang
            'total_harga' => 'required|numeric|min:0|max:99999999.99', // Bisa diubah admin jika ada diskon dll.
            'status_pembayaran' => ['required', Rule::in(['pending', 'paid', 'failed', 'expired'])],
            'reminder_sent' => 'boolean', // Ini biasanya checkbox
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'reminder_sent.boolean' => 'Status pengingat tidak valid.',
        ];
    }
}